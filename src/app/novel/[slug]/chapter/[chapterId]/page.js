...
export default function ChapterPage() {
  ...
  const shareText = ebook ? `Baca "${ebook.title}" di Jeda Renung` : ''
    ...
    <main className={`${styles.main} ${darkMode ? styles.dark : ''}`}>
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Link href={`/novel/${ebookSlug}`} className={styles.backBtn}>← Kembali</Link>
      ...
      <div className={styles.layout}>
        <div className={styles.readerContainer}>
          ...
          <nav className={styles.chapterNav}>
            {prevChapter
              ? <Link href={`/novel/${ebookSlug}/chapter/${prevChapter.id}`} className={styles.navBtn}>← Bab {prevChapter.chapter_number}</Link>
              : <span />}
            <Link href={`/novel/${ebookSlug}`} className={styles.navBtnOutline}>Daftar Bab</Link>
            {nextChapter
              ? <Link href={`/novel/${ebookSlug}/chapter/${nextChapter.id}`} className={styles.navBtn}>Bab {nextChapter.chapter_number} →</Link>
              : <span />}
          </nav>
          ...
          {relatedEbooks.length > 0 && (
            <div className={styles.relatedBox}>
              ...
              <div className={styles.relatedGrid}>
                {relatedEbooks.map(r => (
                  <Link key={r.id} href={`/novel/${r.slug || r.id}`} className={styles.relatedCard}>
              ...
              <div className={styles.tocPopup}>
                ...
                <div className={styles.drawerList}>
                  {allChapters.map(c => (
                    <Link
                      key={c.id}
                      href={`/novel/${ebookSlug}/chapter/${c.id}`}
      ...
      {drawerOpen && (
        <div className={styles.mobileOverlay} onClick={() => setDrawerOpen(false)}>
          <div className={styles.mobileToc} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.drawerList}>
              {allChapters.map(c => (
                <Link key={c.id} href={`/novel/${ebookSlug}/chapter/${c.id}`}
                  className={`${styles.drawerItem} ${c.id === params.chapterId ? styles.drawerActive : ''}`}
                  onClick={() => setDrawerOpen(false)}
                >
                  <span className={styles.sidebarNum}>Bab {c.chapter_number}</span>
